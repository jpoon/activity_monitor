...
void rtlink_tx(rtlink_packet_t *pkt) {
    if( rtl_tx_pkt_check( RTL_TX_SLOT ) != 0 ) {
        ...
        for(uint8_t i = pkt->len+PKT_DATA_START-1; i >= PKT_DATA_START; i--) {
            pkt->payload[i] = pkt->payload[i-PKT_DATA_START];
        }

        rtl_tx_pkt( pkt->payload, pkt->len , RTL_TX_SLOT );
        rtlink_print_packet(pkt);
...
void rtlink_print_packet(const rtlink_packet_t *pkt) {
    printf( "rtl: slot %d, rssi %d, length %d: ", 
        pkt->slot, pkt->rssi, pkt->len );
    for(uint8_t i=PKT_DATA_START; i<pkt->len+PKT_DATA_START; i++ ) {
