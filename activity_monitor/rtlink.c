...
uint8_t rtlink_tx_buf[MAX_RTL_PKT_SIZE];
...
void rtlink_tx(uint8_t *pkt, uint8_t len) {
    if( rtl_tx_pkt_check( RTL_TX_SLOT ) != 0 ) {
        ...
        for(uint8_t i = len+PKT_DATA_START-1; i >= PKT_DATA_START; i--) {
            pkt[i] = pkt[i-PKT_DATA_START];
        }

        rtl_tx_pkt( pkt, len+PKT_DATA_START, RTL_TX_SLOT );
        for(uint8_t i = 0; i < len+PKT_DATA_START; i++) {
            printf("%d %c\r\n", i, pkt[i]);
        }
        printf("\r\n");
